...
def ml_loop(side: str):
    ...
    while True:
        ...
        if not ball_served:
            ...
            dir_x = posOrNeg(scene_info['ball'][0] - lastpos_x) #1向右 or -1向左
            dir_y = posOrNeg(scene_info['ball'][1] - lastpos_y) #1向下 or -1向上
                
            if scene_info['ball'][0] - lastpos_x != ball_Vx:
                rebound_x = 1
                ball_Vx = -ball_Vx
            else:
                rebound_x = 0
            if scene_info['ball'][1] - lastpos_y != ball_Vy:
                rebound_y = 1  
                ball_Vy = -ball_Vy
            else:
                rebound_y = 0
            ...
            if rebound_y != 0:
                ...
                times = (int)(math.ceil((y_temp - 80) / abs(ball_Vy)))
                predict_ballx = x_temp + times * ball_Vx
            if predict_ballx < 0:
                times = (int)(math.floor((0 - predict_ballx) / abs(ball_Vx)))
                predict_ballx = abs(ball_Vx) * times
            if predict_ballx > 195:
                times = (int)(math.floor((predict_ballx - 195) / abs(ball_Vx)))
                predict_ballx = 195 - abs(ball_Vx) * times
                predict_platX = predict_ballx - 15

            if rebound_x != 0:
                x_temp = scene_info['ball'][0]
                y_temp = scene_info['ball'][1]
                times = (int)(math.ceil((y_temp - 80) / abs(ball_Vx)))
                predict_ballx = x_temp + times * ball_Vx
            if predict_ballx < 0:
                times = (int)(math.floor((0 - predict_ballx) / abs(ball_Vx)))
                predict_ballx = abs(ball_Vx) * times
